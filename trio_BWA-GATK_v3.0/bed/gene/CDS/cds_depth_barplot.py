...
def cds_plot(gene, sn):
    ...
    exonStart = []
    exonEnd = []
    ofile = open(sn+'_'+gene+'_cds.depth.csv', 'w')
    ofile.write('sample,gene,CDS,hgmd,chromosome,exonStart,exonEnd,depth\n')
    with open('/DATA/sslyu/trio_BWA-GATK_v3.0/bed/gene/CDS/'+gene+'.bed') as f:
        n = 0
        for l in f:
            l = l.strip('\n').split('\t')
            hgmd = l[-1]
    ...
    for i in range(len(exonStart)):
        ofile.write(sn+','+gene+','+cds[i]+','+hgmd+','+chromosome[i]+','+exonStart[i]+','+exonEnd[i]+','+av_depth[i]+'\n')
    ofile.close()
