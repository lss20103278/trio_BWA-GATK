...
def data_transform(filt3):
    for i in ['GTinfo','GT','Qual','Depth']:
        columns = [j for j in filt3.columns if j+'_' in i] # j+'_', in case of column Trio_GTinfo 
        filt3[i] = filt3[columns].apply(lambda x:';'.join(x.astype(str)), axis=1)
        for k in columns:
            filt3.drop(k, axis=1)
    return filt3
...
def hyperlink_index(list):
    i = list.index('gnomAD_link')+1
    a = i//26
    b = i%26
    if a>1 and b==0:
        ix = chr(64+a-1)+'Z'
        ...
        ix = chr(64+a)+chr(64+b)
    return ix

def xlsxgen(sn,ver,c_f_m):
    print "creating xlsx for sample : %s" %(sn)
    wb=pd.ExcelWriter(sn.strip()+"_"+ver+".xlsx",engine='openpyxl')
    readme = pd.read_excel('/DATA/sslyu/trio_BWA-GATK_v3.0/doc/readme.xlsx',sheetname = 'readme',header = 0)
    ...
    for i in range(2,len(filt3)+1):
        i = str(i)
        list = filt3.columns.tolist()
        ix = hyperlink_index(list)
        ws[ix+i].hyperlink = "http://gnomad.broadinstitute.org/variant/"+ws[ix+i].value
    wb.save()
...
def xlsxgen_children_hospital(sn,ver,c_f_m):
    ...
    for i in range(2,len(filt3)+1):
        ...
        ws[ix+i].hyperlink = "http://gnomad.broadinstitute.org/variant/"+ws[ix+i].value
        #if c_f_m == 'c':
        #    ws['AB'+i].hyperlink = "http://gnomad.broadinstitute.org/variant/"+ws['AB'+i].value
        #elif c_f_m == 'c_f' or c_f_m == 'c_m':
        #    ws['AC'+i].hyperlink = "http://gnomad.broadinstitute.org/variant/"+ws['AC'+i].value
        #elif c_f_m == 'c_f_m':
        #    ws['AD'+i].hyperlink = "http://gnomad.broadinstitute.org/variant/"+ws['AD'+i].value # column AD gnomAD_link
        #else:
        #    ws['AE'+i].hyperlink = "http://gnomad.broadinstitute.org/variant/"+ws['AE'+i].value
    wb.save()
