...
samples=`awk '{print $2}' ../../ped/$sn.ped`
...
echo "combine `date`" >> finished
fi 
 
##cmd:vqsr
if [ "`echo $cmd|sed "s/ /\n/g"|awk '$1=="vqsr"{print "on"}'`" = "on" ];then
#sh vfilt.sh $outdir $sn $filtmode
sh $basedir/vfilt_gatk4.sh $outdir $sn $filtmode
echo "vqsr `date`" >> finished
fi

##cmd:CalculateGenotypePosteriors
if [ "`echo $cmd|sed "s/ /\n/g"|awk '$1=="CalculateGenotypePosteriors"{print "on"}'`" = "on" ];then
/DATA/sslyu/soft/gatk-4.1.0.0/gatk CalculateGenotypePosteriors -R $ref_genome -V $outdir/$sn.raw.snp.pass.vcf -ped ../../ped/$sn.ped -O $outdir/$sn.snp.gt.vcf # can't recognise Chinese

/DATA/sslyu/soft/gatk-4.1.0.0/gatk CalculateGenotypePosteriors -R $ref_genome -V $outdir/$sn.raw.indel.pass.vcf -ped ../../ped/$sn.ped -O $outdir/$sn.indel.gt.vcf
echo "CalculateGenotypePosteriors `date`" >> finished
fi

##cmd:lowGQ
if [ "`echo $cmd|sed "s/ /\n/g"|awk '$1=="lowGQ"{print "on"}'`" = "on" ];then
/DATA/sslyu/soft/gatk-4.1.0.0/gatk VariantFiltration -R $ref_genome --genotype-filter-expression "DP<20 && GQ<20" --genotype-filter-name "lowGQ" -V $outdir/$sn.snp.gt.vcf -O $outdir/$sn.snp.gt.flt.vcf

/DATA/sslyu/soft/gatk-4.1.0.0/gatk VariantFiltration -R $ref_genome --genotype-filter-expression "DP<20 && GQ<20" --genotype-filter-name "lowGQ" -V $outdir/$sn.indel.gt.vcf -O $outdir/$sn.indel.gt.flt.vcf
...
#/DATA/sslyu/soft/gatk-4.1.0.0/gatk VariantAnnotator -L $panel -R $ref_genome -A PossibleDeNovo -ped ../../ped/$sn.ped -V $outdir/$sn.snp.gt.flt.vcf -O $outdir/$sn.snp.gt.novo.vcf # Pedigree argument "pedigree" or "founder-id" was specified without a pedigree annotation being requested, (eg: InbreedingCoeff, ExcessHet) 
#/DATA/sslyu/soft/gatk-4.1.0.0/gatk VariantAnnotator -L $panel -R $ref_genome -A PossibleDeNovo -ped ../../ped/$sn.ped -V $outdir/$sn.snp.gt.flt.vcf -O $outdir/$sn.snp.gt.novo.vcf 
...
#/DATA/sslyu/soft/gatk-4.1.0.0/gatk GatherVcfs -R $ref_genome -I $outdir/$sn.snp.gt.novo.vcf -I $outdir/$sn.indel.gt.novo.vcf -O $sn.gt.vcf  # indel.vcf and snp.vcf must be in genomic order, first record in indel.vcf is not after first record in snp.vcf
/DATA/sslyu/soft/gatk-4.1.0.0/gatk SortVcf -R $ref_genome -I $outdir/$sn.snp.gt.novo.vcf -I $outdir/$sn.indel.gt.novo.vcf -O $sn.gt.vcf
echo "denovo_anotation `date`" >> finished
fi

##cmd:merge (reanalyse etc)
...
sh $basedir/vep-mendelscan.sh $sn ../../ped/$sn.mendel.ped
...
/DATA/sslyu/soft/annovar/convert2annovar.pl -format vcf4 -allsample -withfreq sep/$sepsn.vcf -outfile tmprun/$sepsn.avinput
...
/DATA/sslyu/soft/gatk-4.1.0.0/gatk IndexFeatureFile -F ../gvcf/$i.g.vcf
fi
done

/DATA/sslyu/soft/gatk-4.1.0.0/gatk CombineGVCFs -R $ref_genome -L $panel --dbsnp $ref_snp -O $outdir/$sn.raw.combine.vcf $gvcf_argument # can't recognise Chinese
/DATA/sslyu/soft/gatk-4.1.0.0/gatk GenotypeGVCFs -R $ref_genome -L $panel --dbsnp $ref_snp  -stand-call-conf 10  -O $outdir/$sn.raw.vcf  -V $outdir/$sn.raw.combine.vcf

cd $outdir
vcf-sort $sn.raw.vcf>$sn.raw.sort.vcf
