...
sh $basedir/vfilt_sentieon.sh $outdir $sn $filtmode
...
#vcf-subset -c $sepsn $sn.gt.vcf > $outdir/$sepsn.vcf # Can't locate Vcf.pm in @INC
perl /DATA/sslyu/soft/vcf-subset -c $sepsn $sn.gt.vcf > $outdir/$sepsn.vcf 
done
...
if [ ! -e ../ped/$sn.ped ]; then
cat ../ped/*.mendel.ped |awk '{OFS="\t"; print "0",$2,$3,$4,$5,$6}' |sort |uniq > ../ped/$sn.ped
fi
...
cd results
/home/ana005/anaconda2/bin/python -m peddy -p 4 --plot --prefix $sn ../$sn.sort.vcf.gz ../../../ped/$sn.ped
