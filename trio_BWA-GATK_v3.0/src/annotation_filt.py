...
# -*- coding: utf-8 -*-
"""
version:3.0_20190122
by:lss
inherited from ykl
update:20190122
"""
ThisVer='v3.0'
Thisreleasetime='2019/4/17'
...
ThisVer=ThisVer+'_'+date
m_path = os.path.split(os.path.abspath(sys.argv[0]))[0]
m_path = "/DATA/sslyu/trio_BWA-GATK_v3.0/src"
...
#database confirm:
database = [kwargs.get('-edge'),kwargs.get('-AF'),"OMIM_selected_ch.csv","OMIM_selected_en.csv","phenotype_annotation_hpoteam_include_chpo_doid_xref.csv","morbidmap","DIDGene.csv","OMIM_Gene-Map-Search_20180329.csv","Phenotype20180329.csv", "gene_disease.9606.tsv","OMIM_selected_en20180330.csv"]
missing = []
for i in database:
    if not os.path.exists(m_path+"/../doc/"+i):
        missing.append(i)
if len(missing) > 0:
    ...
    OMIM_Phenotype: %s
    """ % (m_path+"/../doc/"+kwargs.get('-edge'),kwargs.get('-l'),m_path+"/../doc/"+kwargs.get('-AF'),m_path+"/../doc/OMIM_selected_ch.csv",m_path+"/../doc/OMIM_selected_en.csv",m_path+"/../doc/phenotype_annotation_hpoteam_include_chpo_doid_xref.csv",m_path+"/../doc/morbidmap",m_path+"/../doc/DIDGene.csv",m_path+"/../doc/OMIM_Gene-Map-Search_20180329.csv",m_path+"/../doc/Phenotype20180329.csv")

AF=pd.read_table(m_path+"/../doc/"+kwargs.get('-AF'),sep="\t",header = None,dtype = str)
AF = AF[[0,2]]
AF.columns = [0,'locAF']
...
def link_process(sn, ds):
    ...
    if len(p_ID_l) == 0:
        ...
        df_link['link'] = df_link[0]+'-'+df_link[6]+'-'+df_link[8]+'-'+df_link[9] # single
        #df_link = df_link.drop([6,8,9],axis = 1) # usage:unknown
        ...
        df_link['link'] = df_link[0]+'-'+df_link[9]+'-'+df_link[11]+'-'+df_link[12]
...
####### deal with the deletion problem to see if the deletion of nuclei acids affects amnio acids #########
    ...
    ds_del_length_VX = ds_del_length[~ds_del_length['AAChange.refGene'].str.contains('[\d]fs')]
    ds_del_length_VX = ds_del_length_VX[ds_del_length_VX['AAChange.refGene'].str.contains('V*X')]
    if ds_del_length_VX.shape[0] > 0:
        #ds_del_length_VX.loc[:,'Deletion_Problem'] = 'note'
        ds_del_length_VX['Deletion_Problem'] = 'note'
    ...
    ds_del_length = ds_del_length[ds_del_length['AAChange.refGene'].apply(lambda x:x.split(',')[0]).str.contains('p.*del')] # CR1:NM_000573:exon30:c.5122delG:p.V1708X
    ...
    if ds_del_length_VX.shape[0] > 0:
        ds_del_length_all_tmp = pd.concat([ds_del_length_fs,ds_del_length])
        ds_del_length_all = pd.concat([ds_del_length_all_tmp,ds_del_length_VX])
    else:
...
def filt1(sn, ds, kwargs):
    ...
    ex_edge=pd.read_csv(m_path+"/../doc/"+kwargs.get('-edge'))
...
def add_other(sn, ds):    
    ...
    gene_disease_9606=pd.read_csv(m_path+"/../doc/gene_disease.9606.tsv",encoding='gbk',dtype=str,sep="\t")
    gene_disease_9606=gene_disease_9606.fillna("NA")
    OMIM_selected_ch=pd.read_csv(m_path+"/../doc/OMIM_selected_ch.csv",encoding='gbk',dtype=str)
    OMIM_selected_en=pd.read_csv(m_path+"/../doc/OMIM_selected_en20180330.csv",encoding='gbk',dtype=str)
    phenon=pd.read_csv(m_path+"/../doc/phenotype_annotation_hpoteam_include_chpo_doid_xref.csv",encoding='gbk',dtype=str)
    phenon=phenon[phenon.infosource=='OMIM']
    phenon=phenon.fillna("NA")
    morbidmap = pd.read_csv(m_path+"/../doc/morbidmap",sep = '\t',names = ['Phenotype','Gene_Symbols','MIM_Number','Cyto_Location','Gene'],encoding = 'gbk',dtype = str) #update
    DIDGene = pd.read_csv(m_path+"/../doc/DIDGene.csv",sep = '\t',names = ['Gene'])
    OMIM_Gene_Map_Search = pd.read_csv(m_path+"/../doc/OMIM_Gene-Map-Search_20180329.csv",header = 0,encoding = 'gbk',dtype = str)
    OMIM_Phenotype = pd.read_csv(m_path+"/../doc/Phenotype20180329.csv",header = 0,encoding = 'gbk',dtype = str)
...
for sn in list:
    ...
    #ds['AF'] = ds['AF'].apply(pd.to_numeric, errors='coerce').fillna('.')
    ...
    ##step4:create xlsx
    print "\nStep4:creating xlsx"
    xlsxgen(sn,ThisVer,c_f_m) # /DATA/sslyu/trio_BWA-GATK_3.0/src/Data_transform_v3.R execute problemly
