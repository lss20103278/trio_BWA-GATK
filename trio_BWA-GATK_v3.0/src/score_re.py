...
indisease=open("/DATA/sslyu/trio_BWA-GATK_v3.0/doc/gene_disease.txt","r")
...
for sn in list:
  ...
  header = inline.readline()
  header = header.split('\t')
  col_ix = {}
  cols = ["CLINSIG", "ensembl", "SIFT_pred", "Polyphen2_HVAR_pred", "MutationTaster_pred", "FATHMM_pred", "M-CAP_pred", "REVEL", "hgmd", "Gene.refGene"]
  for i in cols:
      col_ix[i] = header.index(i)
  print col_ix
  inline=open(sn+".ann.hg19_multianno.txt","r")
...
####### calculate score_clinvar ###########################################
    ...
    if p1.search(line.strip().split('\t')[col_ix["CLINSIG"]]): #Pathogenic, annovar 2018 line[123]: column CLINSIG
      s2[0]=1
    #if p2.search(line.strip().split('\t')[123]): #Likely pathogenic
    if p2.search(line.strip().split('\t')[col_ix["CLINSIG"]]): #Pathogenic, annovar 2018 line[123]: column CLINSIG
      s2[1]=1
    #if p3.search(line.strip().split('\t')[123]): #Likely benign
    if p3.search(line.strip().split('\t')[col_ix["CLINSIG"]]): #Pathogenic, annovar 2018 line[123]: column CLINSIG
      s2[2]=1
    #if p4.search(line.strip().split('\t')[123]): #Benign
    if p4.search(line.strip().split('\t')[col_ix["CLINSIG"]]): #Pathogenic, annovar 2018 line[123]: column CLINSIG
      s2[3]=1
...
####### calculate score_ensembl ############################################ 
    ...
    if p5.search(line.strip().split('\t')[col_ix["ensembl"]]): #CLIN_pathogenic line[124]: column ensembl
      s3[0]=1
    #if p6.search(line.strip().split('\t')[124]): #CLIN_likely_pathogenic
    if p6.search(line.strip().split('\t')[col_ix["ensembl"]]): #CLIN_pathogenic line[124]: column ensembl
      s3[1]=1
    #if p7.search(line.strip().split('\t')[124]): #CLIN_risk_factor
    if p7.search(line.strip().split('\t')[col_ix["ensembl"]]): #CLIN_pathogenic line[124]: column ensembl
      s3[2]=1
    #if p8.search(line.strip().split('\t')[124]): #CLIN_association
    if p8.search(line.strip().split('\t')[col_ix["ensembl"]]): #CLIN_pathogenic line[124]: column ensembl
      s3[3]=1
    scoreensembl=max(map(lambda x, y: x*y,  s1, s3))

###### calculate score_pre ##################################################
    #if line.strip().split('\t')[25]=="D": # SIFT_pred
    if line.strip().split('\t')[col_ix["SIFT_pred"]]=="D": # SIFT_pred
      scorepre=scorepre+1
    #if line.strip().split('\t')[31]=="D": # Polyphen2_HVAR_pred
    if line.strip().split('\t')[col_ix["Polyphen2_HVAR_pred"]]=="D": # Polyphen2_HVAR_pred
      scorepre=scorepre+1
    #if line.strip().split('\t')[37]=="D": # MutationTaster_pred
    if line.strip().split('\t')[col_ix["MutationTaster_pred"]]=="D": # MutationTaster_pred
      scorepre=scorepre+1
    #if line.strip().split('\t')[43]=="D": # FATHMM_pred
    if line.strip().split('\t')[col_ix["FATHMM_pred"]]=="D": # FATHMM_pred
      scorepre=scorepre+1
    #if line.strip().split('\t')[57]=="D": # M-CAP_pred
    if line.strip().split('\t')[col_ix["M-CAP_pred"]]=="D": # M-CAP_pred
    	scorepre=scorepre+1
    #if line.strip().split('\t')[89] != 'NA': # REVEL
    if line.strip().split('\t')[col_ix["REVEL"]] != 'NA': # REVEL
      #if float(line.strip().split('\t')[89])>0.5:
      if float(line.strip().split('\t')[col_ix["REVEL"]])>0.5:
        scorepre=scorepre+1
...
#    if line.split('\t')[0]+','+line.split('\t')[1] in hgmd.keys():
    ...
    if (line.split('\t')[col_ix["hgmd"]] not in confhgmd): # hgmd
      scorehgmd=5
    
    #if line.split('\t')[6].split(',')[0] in diseaseid.keys(): # Gene.refGene
    if line.split('\t')[col_ix["Gene.refGene"]].split(',')[0] in diseaseid.keys(): # Gene.refGene
